...
- **Ask AI** — eight tools so the queue can be asked about and driven in natural language: what is failing, why a task failed, how far through a group is, and creating a group, queueing, killing, restarting failures, and cleaning up. Ships example prompts so the capability is discoverable rather than remembered. Every tool that changes anything confirms first, naming the tasks and the daemon it will act on. Unknown connection names are refused rather than silently resolved to this machine, because task ids are per-daemon. `reset` is deliberately not exposed.
